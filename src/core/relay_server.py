...
服务器回应 OK: [1字节房间码长度] [房间码 ASCII] [2 字节 UDP端口, 0=禁用]
流量包: CMD_DATA [2 字节 len] [data]
UDP 包: CMD_UDP [2 字节 player_id] [data]
"""
...
class RelayRoom:
    def __init__(self, code: str, host_writer: asyncio.StreamWriter, host_flags: int = 0):
        ...
        self._next_pid = 1
        self.udp_sessions: Dict[Tuple[str, int], int] = {}
...
class RelayServer:
    ...
    async def _handle_host(self, reader, writer, data: bytes):
        ...
        resp = bytes([len(code_bytes)]) + code_bytes + struct.pack(">H", udp_port)
        await _send_pkt(writer, CMD_OK, resp)
        logger.info(
            f"[RELAY] 房间 {code} 已创建"
            f"（陶瓦={room.has_terracotta}, UDP={udp_port or '禁用'}）"
        )
    ...
    async def _handle_player(self, reader, writer, data: bytes):
        ...
        await _send_pkt(writer, CMD_OK, resp)
        logger.info(
            f"[RELAY] 玩家 {pid} 加入 {code}"
            f"（陶瓦={bool(flags & FLAG_TERRACOTTA)}）"
        )

        try:
            await _send_pkt(
                room.host_writer, CMD_PLAYER_JOINED, bytes([pid, flags])
            )
        except Exception:
            pass
        ...
        try:
            ...
            try:
                await _send_pkt(
                    room.host_writer, CMD_PLAYER_LEFT, bytes([pid])
                )
            except Exception:
                pass
            writer.close()
...
class _UdpProtocol(asyncio.DatagramProtocol):
    ...
    async def _forward(self, room: RelayRoom, pid: int, data: bytes):
        try:
            pkt = (
                struct.pack(">BH", CMD_UDP, 2 + len(data))
                + struct.pack(">H", pid)
                + data
            )
